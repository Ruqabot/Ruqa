...
export async function notInVC(message: Message): Promise<boolean> {
    if (!message.member?.voiceState.channelID) {
        await buildMsg(message, "You need to be in a voice channel before executing this command.");
        return false;
    }
    return true;
}
...
export async function onlyAllowedRequester(
    ...
    if ((player!.current!.requester as User).id as string !== interaction.member?.id) {
        await interaction.createMessage({ embeds: [new RichEmbed().setColor(RichEmbed.embedColor).setDescription("Only requester can use this buttons.")], flags: 64 });
        return false;
    }
    return true;
}

export async function configuredDJRole(
    message: Message,
): Promise<boolean> {
...
export async function isPlayerActive(message: Message): Promise<[Player | undefined, boolean]> {
    const audio = ruqa.audio.players?.get(message.guildID!);
    if (!audio || !audio?.current) {
        await message.channel.createMessage({ embeds: [new RichEmbed().setColor(RichEmbed.embedColor).setDescription("I'm nothing playing right now.")] });
        return [audio, false];
    }
    return [audio, true];
}
