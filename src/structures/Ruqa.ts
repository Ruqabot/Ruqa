...
export default class Ruqa extends Client {
    ...
    private async loadCommands(): Promise<this | undefined> {
        const directories = await promises.readdir(resolve("dist", "commands"), { withFileTypes: true });
        for (const directory of directories) {
            if (!directory.isDirectory) {
                return;
            }
            const commands = await promises.readdir(resolve("dist", `commands/${directory.name}`), { withFileTypes: true });
    ...
    private async loadGuildEvents(): Promise<void> {
        const discordEvents = await promises.readdir(resolve("dist", "listeners", "Guild"));
    ...
    private async loadLavalinkEvents(): Promise<void> {
        const discordEvents = await promises.readdir(resolve("dist", "listeners", "Lavalink"));
        for (const event of discordEvents) {
