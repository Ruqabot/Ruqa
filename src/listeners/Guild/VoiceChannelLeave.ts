...
export default new Event("voiceChannelLeave", async (member: Member, oldChannel: VoiceChannel) => {
    ...
    if (!member.bot && player.voiceChannelId === oldChannel.id
    && oldChannel.voiceMembers.filter((x) => x.bot).length) {
        ...
        const msg = await ruqa.createMessage(player.textChannelId!, {
            embeds: [
                ...
                .setDescription("All left me alone, I'll take my leave in 10 seconds."),
            ],
        });
