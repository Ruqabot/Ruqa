    ...
    if (interaction instanceof ComponentInteraction) {
        ...
        switch (interaction.data.custom_id) {
            case "pauseOrResume":
                ...
                if (player?.paused) {
                    player?.pause(false);
                    msg = await ruqa.getMessage(interaction.channel.id, interaction.message.id);
                    await msg.edit({
                        components: [{
            ...
            case "skip":
                ...
                if (player!.queue.length < 1) {
                    await interaction.createFollowup({ embeds: [new RichEmbed().setColor(RichEmbed.embedColor).setDescription("Queue must have at least **1 track** to skip.")], flags: 64 });
                    return;
                }
                msg = await ruqa.getMessage(interaction.channel.id, interaction.message.id);
                player?.skip();
        ...
        switch (interaction.data.custom_id) {
            ...
            case "np_refresh":
                ...
                .setDescription(`\n\n**Author**: ${player?.current?.author}\n**Requester**: <@!${(player?.current?.requester as User).id}>\n**Duration**: ${prettyMs(Number(player?.current?.duration), { verbose: true })}`)
                .addField(
                    "Upcoming",
                    player?.queue[0]
        ...
        switch (interaction.data.custom_id) {
            case "q_previousStart":
                ...
                await ruqa.cacheQueueMsg.edit({
                    embeds: [new RichEmbed().setDescription(`**Queued Tracks**\n\n${ruqa.pages[ruqa.page]}`).setColor(RichEmbed.embedColor).setFooter(`Page ${ruqa.page + 1}/${ruqa.pages.length}`)],
                });
            break;
            ...
            case "q_forwardEnd":
                ...
                ruqa.page = ruqa.page + 1 < ruqa.pages.length ? ruqa.pages.length - 1 : 0;
                await ruqa.cacheQueueMsg.edit({
                    embeds: [new RichEmbed().setDescription(`**Queued Tracks**\n\n${ruqa.pages[ruqa.page]}`).setColor(RichEmbed.embedColor).setFooter(`Page ${ruqa.page + 1}/${ruqa.pages.length}`)],
                });
            break;
