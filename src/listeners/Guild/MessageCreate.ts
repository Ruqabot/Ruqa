import type { Message, PossiblyUncachedTextableChannel, TextableChannel } from "eris";
...
export default new Event("messageCreate", async (message: Message<PossiblyUncachedTextableChannel>) => {
    ...
    Prefix.findOne({
        ...
        || message.content.startsWith(`<@!${client.user.id}>`)) {
            ...
            await (channel as TextableChannel)
            .createMessage({ embeds: [new RichEmbed().setColor(RichEmbed.embedColor).setDescription(`My prefix for this server is **${prefix}**`)] });
            return;
        }
        ...
        if ((cmd as { isDisabled?: boolean })?.isDisabled) {
            ...
            .createMessage({
                embeds: [
                    ...
                    .setDescription(`${command?.toString().charAt(0).toUpperCase() + command.slice(1)} command was been disabled globally, for more information please join our [Discord](https://discord.gg/${env.DISCORD_SERVER_INVITE_CODE!}) server.`),
                ],
            });
            return;
        }
