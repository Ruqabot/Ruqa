import { type Client, Constants, type Message, type User } from "eris";
      ...
      .setDescription(
        `\n\n**Author**: ${player?.current?.author}\n**Requester**: <@!${
          ...
          player?.current?.isStream
            ? "Stream"
            : prettyMs(Number(player?.current?.duration), { verbose: true })
        }`
      )
      .addField(
        "Upcoming",
      ...
      components: [
        {
          ...
          components: player?.paused ? nowplayingModifier : nowplayingButtons,
        },
      ],
    });
  },
