...
export default new Command({
    ...
    run: async ({ client, message, args }: {
        ...
        if (!player.playing || player.paused) {
            await buildMsg(message, "Current track is either paused or not stucked, can't seek.");
            return;
        }
        if (time < 1 || time > player.current.duration) {
            await buildMsg(message, `Seek time must be bigger than **1 sec** and must be smaller than **${ms(player.current.duration, { long: true })}**.`);
            return;
        }
        if (typeof time === "undefined") {
            await buildMsg(message, "Uhhh, most likely an error occurred to seek, can you try something else, like different time format?");
            return;
        }
        player.seek(time);
