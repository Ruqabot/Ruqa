  ...
  run: async ({
    ...
    if (!args[0]) {
      const embed = new RichEmbed()
        ...
        .setDescription(
          `Hi! I'm ${client.user.username}. A very simple and easy to use Discord bot.`
        )
        .setThumbnail(client.user?.avatarURL ?? client.user.defaultAvatarURL)
        .addField(
          `• Music (${(await promises.readdir("dist/commands/Music")).length})`,
          `\`${(
            await promises.readdir("dist/commands/Music", {
              withFileTypes: true,
            })
          )
            .map((x) => x.name.replace(".js", "").toLowerCase())
            .join(", ")}\``
        )
        .addField(
          `• Filters (${
            (await promises.readdir("dist/commands/Filters")).length
          })`,
          `\`${(
            await promises.readdir("dist/commands/Filters", {
              withFileTypes: true,
            })
          )
            .map((x) => x.name.replace(".js", "").toLowerCase())
        ...
        .addField(
          ...
          `\`${(
            await promises.readdir("dist/commands/Info", {
              withFileTypes: true,
            })
          )
            .map((x) => x.name.replace(".js", "").toLowerCase())
      ...
      if (!cmd) {
        message.channel.createMessage({
          content: "Mentioned command not found.",
        });
        return;
      }
      const embed = new RichEmbed()
        ...
        .addField(
          "Aliases",
          `\`${
            (cmd as { aliases: Array<string> }).aliases
              .map((x: string) => x)
              .join(", ") ?? "None"
          }\``,
          true
        ...
        .addField(
          "Category",
          `\`${(cmd as { category?: string }).category ?? "None"}\``,
          true
        )
        .addField(
          "Description",
          `\`${(cmd as { description?: string }).description ?? "None"}\``,
          true
        );
