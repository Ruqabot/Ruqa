...
export default new Command({
    ...
    run: async ({ message, args }: {
        ...
        if (!code) {
            await buildMsg(message, "You need to provide some code. Don't abuse else ghost will take you.");
            return;
        }
        if (!code.startsWith("```js\n") || !code.endsWith("\n```")) {
            await buildMsg(message, "Code must be inside JavaScript code blocks with new line.");
            return;
        }
        try {
