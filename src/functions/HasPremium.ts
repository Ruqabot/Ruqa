...
export default async function hasPremium(
  ...
  const member = await ruqa.getRESTGuildMember(
    env.BOT_SUPPORT_GUILD_ID!,
    memberID
  );
  ...
  if (!member.roles.find((x) => x === env.PREMIUM_ROLEID)) {
    await buildMsg(
      ...
      `${
        Emojis[0].star
      } This command is only for premium members, it only $0.5 / month, for more information please join our [Discord](https://discord.gg/${
