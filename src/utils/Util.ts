...
export function buildProgressBar(total?: number, current?: number): [string, number] {
    ...
    const progressText = progress >= 1 ? `[${barText.repeat(progress)}](https://discord.gg/${env.SUPPORT_SERVER ?? "not_set"})${barSlider}` : barSlider;
...
export function makeChunk(array: Array<string> | string, size: number = 1): Array<string[]> {
