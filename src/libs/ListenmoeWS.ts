...
export class ListenmoeWS {
    ...
    public heartBeat(waitTime: number): void {
        this.timeHeartBeat = setTimeout(() => this.#ws!.send(JSON.stringify({ op: 9 })), waitTime);
    }
    ...
    public onMessage(data: string): ListenMoeData | undefined {
        ...
        if (msg?.t !== "TRACK_UPDATE"
        && msg?.t !== "TRACK_UPDATE_REQUEST") {
            return undefined;
        }
        ...
        this.data = {
            ...
            ? msg.d.song?.artists.map((x: { nameRomaji: string; name: string; id: string | number; }) => `[${x.nameRomaji || x.name}](https://listen.moe/music/artists/${x.id})`).join(", ")
            : undefined,
            ...
            ? msg.d.song?.sources.map((x: { nameRomaji?: string; name: string; }) => x?.nameRomaji ?? x?.name).join(", ")
            : undefined,
            albums: msg.d?.song?.albums && msg.d.song.albums?.length
            ? msg.d.song?.albums.map((x: { name: string; id: string | number; }) => `[${x?.name}](https://listen.moe/music/albums/${x?.id})`).join(", ")
            : undefined,
            cover: msg.d?.song?.albums && msg.d.song.albums?.length && msg.d.song?.albums[0]?.image
