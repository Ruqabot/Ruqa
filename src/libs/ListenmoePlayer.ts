...
export default class ListenmoePlayer {
    ...
    public async connectListenMoe(pop: string): Promise<void> {
        const selectPop = pop === "jpop" ? "https://listen.moe/stream" : "https://listen.moe/kpop/stream";
    ...
    public async playListenMoe(): Promise<void> {
        switch (this.#res?.loadType) {
            case "LOAD_FAILED":
                await buildMsg(this.#message, "Something broke while loading the track/playlist.");
            break;
        ...
        if (!this.#player!.playing) {
            ...
            await buildMsg(this.#message, "Currently, I'm playing, to queue other you can use the play command.");
        }
    }
}
